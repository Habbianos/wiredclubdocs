...
O ecossistema Wired é dividido em sete categorias, identificadas visualmente pela cor dos mobis. Cada categoria tem um papel específico na arquitetura de uma pilha.

| Nome                     | Descrição |
|--------------------------|----------|
| Ativadores               | Detectores de eventos. Monitoram o quarto e iniciam a execução quando uma condição externa é detectada: um passo, uma palavra-chave, alguém entrando no quarto. |
| Efeitos                  | Executores. Traduzem a sequência lógica em uma mudança perceptível: mover mobis, teletransportar usuários, exibir mensagens ou expulsar jogadores. |
| Condições                | Filtros lógicos. Não executam ações, mas determinam se a execução deve ocorrer. Suportam lógica positiva, negativa/invertida (em alguns casos) e OU através do uso de outro Wired Extra. |
| Seletores                | Coletadores. Definem alvos dinamicamente por área, tipo, variável ou sinal remoto, sem seleção manual ou limite de ~~5~~ 20 itens por caixa. |
| Extras                   | Modificadores. Alteram o comportamento de diversos fatores, como a ordem de execução dos Efeitos, física que deve ser aplicada ou a criação de campos de texto personalizados. |
| Variáveis                | Armazenamento de dados. Armazenam informações das mais diversas, além de expôr informações internas do jogo como estado ou direção de um mobi, hora do dia, por quanto tempo o quarto está ativo e muito mais. |
| Contratos | Regras de transação. Definem os termos de uma negociação, especificando o que deve ser entregue, recebido ou recompensado para que a transação seja considerada válida e concluída dentro do sistema. |
